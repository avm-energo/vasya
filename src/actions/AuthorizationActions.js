...
export const login = async (login, password, callback) => {
    try {
        ...
        if (!text.StatusCode){
            ...
            localStorage.setItem('userPermissions', JSON.stringify(text.client.permissions))
            // store.dispatch('setRole_action', text.role)
            store.dispatch('setRole_action', 'Administrator')
            ...
            store.dispatch('setUserPermissions_action', text.client.permissions)
    ...
    try {
        ...
        if (text.status == 200){
            // if (localStorage.getItem('userName') != 'Guest') {
                store.dispatch('setIsAuth_action', true)
                store.dispatch('setRole_action', localStorage.getItem('role'))
                ...
                store.dispatch('setUserPermissions_action', JSON.parse(localStorage.getItem('userPermissions')))
            // } else {
            //     store.dispatch('setIsAuth_action', true)
...
export const logout = async (callback) => {
    try {
        ...
        localStorage.removeItem('userPermissions')
        store.dispatch('setIsAuth_action', false)
        store.dispatch('setRole_action', null)
